 ...
 */class Weaver{/**
   ...
   * @returns {this} this
   */constructor(a){return _defineProperty(this,"showResults",a=>{const b=Object.keys(a);return this.logging(`Found interlaced dataEntries: ${b.join(", ")}`),this.logging(`Total dataEntries: ${b.length}`),Promise.resolve(a)}),_defineProperty(this,"connectClients",a=>{const b=a.map(a=>a.connect());return Promise.all(b).catch(this.logging)}),_defineProperty(this,"disconnect",async a=>{const b=a=>{let b=0;const c=this.dataClients.length;return d=>{b+=1,b===c&&a(d)}},c=b(a);await this.dataClients.forEach(async a=>{await a.disconnect(!0,c)}),this.logging(`disconnected ${this.dataClients.length} clients...`)}),_defineProperty(this,"run",a=>{const b=this.connectClients(this.dataTargets).then(()=>this.connectClients(this.dataSources)).then(()=>this.collect.runQueries(this.queries)).then(this.collect.interlace).then(this.showResults).then(this.digest.saveJSON).then(this.digest.dump).catch(this.logging).then(a);return b}),this.__cache={},this.collect=new WeaverCollect(a),this.digest=new WeaverDigest(a),this._configure(a)}/**
   *
